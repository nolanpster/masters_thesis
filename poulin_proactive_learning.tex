...
\begin{abstract}
    ...
    improve the likelihood of a reward by correctly inferring uncontrollable agent's policy, but the actions that
    provide the most information for inference decrease the chance of reward. By balancing exploration and exploitation,
    the controllable agent improves the likelihood of a reward as it builds up a history of interactions with
    uncontrollable agent. The policy of the uncontrollable agent is modeled as a Softmax policy, and its Q-function is
    approximated using a linear combination of basis-functions and a parameter vector. Assuming that the the optimal
    parameter vector is the mean value of a multivariate-normal distribution, the uncontrollable policy is inferred by
    performing gradient ascent that maximizes the log-likelihood of observed data given sampled parameter vectors. This
