...
\subsection{Preliminaries}\label{sec:policy_iteration_preliminaries}
    The parameterization for \policy{2} requires an understanding of how a reward function, $R$, influences the
...
\subsection{Q-function Approximation}\label{sec:policy_parameterization}
    To build an estimate of \agent{2}'s policy, $\estimate{\policy{}}_2$,  we'll parameterize \agent{2}'s
    ...
    and \cite{akiyama2010efficient}. It is advantageous that \agent{1} does not need to learn the reward function of
...
\subsubsection{Algorithm Termination}\label{sec:policy_infer_terminate}
    ...
    \policy{2} is set to be uniform across the entire action set available to \agent{2}. We'll set the following
    parameters:
...
\todo[inline]{Add results from smallest data-set.}
    \begin{table}[H]
        \centering
        \begin{tabular}{c|l l}
