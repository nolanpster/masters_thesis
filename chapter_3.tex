...
\subsubsection{Policy Solution}
    ...
    optimal action at each state. This is known as \acf{VI}. Note that if we use the true policy of \agent{2}, an
...
clustering algorithm \cite{something}, the resulting policy is not a hardmax (deteriministic) like Eq.
...
\end{assumption}
    ...
    Now, consider that the $K^{\text{th}}$ kernel is mobile; it is attached to the \agent{1}\!'s location. When this
    kernel is far from the current state of \agent{2} its anticipated influence is very small due to Eq.
...
\end{table}
    ...
    \agent{1} is located in the blue cell. The effect of the mobile kernel is obvious in comparison to Figure.
...
In future experiments, \agent{1} will be tasked to reach the upper left ($North-West$) grid cell, $s_1=0$,
...
$s$ from the true \policy{2}. For instance, if $Error(n)==1$, the $\estimate{\policy{2}}$ will select a different action
...
Given that $W \ll |S|\times |A|$, the inference result in Fig. \ref{fig:multi_agent_error_25_kernels} produced a
